  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if (cpuControlled && !ClientPrefs.communityGameBot)
		{
			if (ClientPrefs.hudType != "Mic'd Up" && ClientPrefs.hudType != 'Box Funkin')
			{
			scoreTxt.text = 'Bot Score: ' + formattedScore + ' | Bot Combo: ' + formattedCombo + npsString + botText;
			if (ClientPrefs.healthDisplay) scoreTxt.text += ' | Health: ' + FlxMath.roundDecimal(health * 50, 2) + '%';
			} else {
				scoreTxt.text = "Bot Score: " + formattedScore;
		...
		else switch (ClientPrefs.hudType)
			{
				...
				case "Mic'd Up", 'Box Funkin':
					...
					scoreTxt.text = 'Score: ' + formattedScore;
					missTxt.text = "Misses: " + formattedSongMisses;
					accuracyTxt.text = 'Accuracy: ' + accuracy + ' | ' + fcString + ' |' + ratingCool;
					npsTxt.text = npsString;

				case "Doki Doki+":
					scoreTxt.text = 'Score: ' + formattedScore + ' | Breaks: ' + formattedSongMisses + ' | Combo: ' + formattedCombo + npsString + ' | Accuracy: ' + accuracy + ' | ' + fcString + ratingCool;

				case "Dave and Bambi":
					scoreTxt.text = 'Score: ' + formattedScore + ' | Misses: ' + formattedSongMisses + ' | Combo: ' + formattedCombo + npsString + ' | Accuracy: ' + accuracy + ' | ' + fcString;
...
if (ClientPrefs.showNPS) {
	...
	if (unspawnNotes[0] != null && (Conductor.songPosition + 1800 / songSpeed) >= firstNoteStrumTime && !ClientPrefs.showNotes && cpuControlled)
	{
		spawnTime /= unspawnNotes[0].multSpeed;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (daRating.name == 'sick' && !ClientPrefs.noMarvJudge) maxScore -= 150 * Std.int(polyphony); //if you enable marvelous judges and hit a sick, lower the max score by 150 points. otherwise it won't make sense
...
if (!allSicks && ClientPrefs.colorRatingFC && shits > 0 && noteDiff > ClientPrefs.marvWindow && ClientPrefs.hudType != 'Tails Gets Trolled V4' && ClientPrefs.hudType != 'Doki Doki+') 
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			if (note.noteType == 'Angel Note')
			{
				...
				final char:Character = !opponentChart ? boyfriend : dad;
	...
	public function updateRatingCounter() {
		...
		formattedNPS = !ClientPrefs.compactNumbers ? FlxStringUtil.formatMoney(nps, false) : compactNPS;
		final formattedMaxNPS = !ClientPrefs.compactNumbers ? FlxStringUtil.formatMoney(maxNPS, false) : formatCompactNumber(maxNPS);
		final formattedOppNPS = !ClientPrefs.compactNumbers ? FlxStringUtil.formatMoney(oppNPS, false) : formatCompactNumber(oppNPS);
		final formattedMaxOppNPS = !ClientPrefs.compactNumbers ? FlxStringUtil.formatMoney(maxOppNPS, false) : formatCompactNumber(maxOppNPS);
		final formattedEnemyHits = !ClientPrefs.compactNumbers ? FlxStringUtil.formatMoney(enemyHits, false) : formatCompactNumber(enemyHits);
		if (!ClientPrefs.noMarvJudge)
		{
